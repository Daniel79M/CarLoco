@extends('layout.base')

<div class="footer_section layout_padding">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="footeer_logo"><img src="images/logo.png"></div>
                
            </div>
        </div>
        <div class="footer_section_2">
            <div class="row">
                <div class="col">
                    <h4 class="footer_taital"></h4>
                    <p class="footer_text">Qui sommes-nous ?
                        Nos offres d'emploi
                        Devenir annonceur pub
                        Devenir partenaire pro</p>
                    <div class="form-group">
                        <textarea class="update_mail" placeholder="Enter Your Email" rows="5" id="comment" name="Enter Your Email"></textarea>
                        <div class="subscribe_bt"><a href="#">Soumettre</a></div>
                    </div>
                </div>
                <div class="col">
                    <h4 class="footer_taital">Information</h4>
                    <p class="lorem_text">Notre service disponible pour vous... </p>
                </div>
                <div class="col">
                    <h4 class="footer_taital">Aide</h4>
                    <p class="lorem_text">Notre service disponible pour vous... </p>
                </div>
                <div class="col">
                    <h4 class="footer_taital">Investissement</h4>
                    <p class="lorem_text">Qui sommes-nous ?
                        Nos offres d'emploi
                        Devenir annonceur pub
                        Devenir partenaire pro </p>
                </div>
                <div class="col">
                    <h4 class="footer_taital">Contacts</h4>
                    <div class="location_text"><a href="#"><i class="fa fa-map-marker"
                                aria-hidden="true"></i><span class="padding_left_15">Location</span></a></div>
                    <div class="location_text"><a href="#"><i class="fa fa-phone"
                                aria-hidden="true"></i><span class="padding_left_15">(+00228) 92286978</span></a>
                    </div>
                    <div class="location_text"><a href="#"><i class="fa fa-envelope"
                                aria-hidden="true"></i><span class="padding_left_15">miryamdegbey20@gmail.com</span></a>
                    </div>
                    <div class="social_icon">
                        <ul>
                            <li><a href="https://www.facebook.com/?locale=fr_FR"><i class="fa fa-facebook" aria-hidden="true"></i></a></li>
                            <li><a href="#"><i class="fa fa-instagram" aria-hidden="true" ></i></a></li>
                            <li><a href="#"><i class="fa-solid fa-linkedin" ></i></a></li>
                            <li><a href="#"><i class="fa-solid fa-instagram" ></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>